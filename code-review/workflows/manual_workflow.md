...
按 [references/review_process.md > 已忽略问题去重与报告归档](references/review_process.md#已忽略问题去重与报告归档) 执行：

- 扫描 `docs/code_reviews/fixed/` 目录下的报告文件。
- 提取标记为「忽略」的问题，去重后追加到 `ignored_issues.md`。
- 扫描完成后将 `fixed/` 下全部报告移动到 `archived/`。
- 若归档失败：记录失败原因并继续审查流程，在最终通知中说明。

### 步骤 5：加载已忽略问题清单

按 [references/review_process.md > 已忽略问题去重与报告归档](references/review_process.md#已忽略问题去重与报告归档) 加载 `ignored_issues.md`，提取文件路径与问题标题供审查去重使用。
