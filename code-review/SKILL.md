...
- **跳过废弃/旧文件**：审查前必须先执行前置过滤，排除用户标注的废弃/旧文件与文件夹。规则详见 `review_process.md > 跳过废弃 / 旧文件与文件夹`。核心要点：
  - 固定命名废弃目录（`deprecated` / `old_file` / `legacy` / `archive` / `bak` / `废弃` / `旧文件` / `遗留` / `归档` / `备份` 等，不区分大小写）自动跳过
  - 用户标记文件 `.code_review_ignore` / `.cr_skip` 中声明的条目自动跳过
  - 文件名以 `deprecated_`、`old_`、`legacy_`、`obsolete_`、`archive_`、`bak_` 前缀开头的文件自动跳过
  - 被过滤内容不进入统计、不进入问题检测、不写入报告；报告头部仅统计实际参与审查的文件
- **已忽略问题去重（防重复报告）**：
  - 每次审查前，先从 `docs/code_reviews/fixed/` 目录扫描用户已整理的报告，提取其中标记为「忽略」的问题，更新到 `ignored_issues.md`
  - 扫描范围严格限定：仅扫描 `docs/code_reviews/fixed/` 目录下的报告文件，不扫描根目录下的新生成报告（避免未处理报告中的标记被误读）
  - 然后加载 `ignored_issues.md`，提取已忽略问题列表供审查过滤使用
  - 审查过程中发现的问题若与已忽略问题清单中的条目在「文件路径 + 问题标题」上匹配（标题相似度 ≥ 80% 或完全一致，且文件路径相同），则跳过该问题，不纳入统计、不写入报告
  ...
  - 如用户需恢复跟踪某已忽略问题，可手动从 `ignored_issues.md` 中删除对应条目，下次审查将重新报告
...
- **忽略清单保存与分发**：`ignored_issues.md` 与审查报告同等对待，保存规则如下：
