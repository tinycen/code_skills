...
| 技能 | 说明 |
|------|------|
| [code-review](code-review/) | 代码审查 - 自动化代码质量检查与报告生成 |
...
├── fixed/                      # 已修复/阅读的报告(由用户手动标注)
├── archived/                   # 已归档的报告（自动整理）
└── ignored_issues.md           # 已忽略问题清单（自动整理）
```

> 流程说明：
> - **根目录报告**：自动审查生成，等待用户查看处理，**不会被工具扫描**（避免未处理报告中标记被误读）。
> - **`fixed/` 目录（收件箱）**：用户查看并确认处理后的报告手动移入此处，作为下一次审查的扫描来源。
> - **`archived/` 目录**：工具扫描 `fixed/` 提取忽略标记后，将 `fixed/` 下**全部报告自动归档**至此，`fixed/` 保持为空，避免对历史报告重复扫描。
> - **`ignored_issues.md`**：由工具在扫描 `fixed/` 时自动生成与维护，跨次审查持久化；审查过程中匹配「文件路径 + 问题标题」的问题将被跳过，不重复报告。

下图说明审查报告从**自动生成**到**自动归档**的完整流转过程，以及「已忽略问题清单」如何自动生成与维护。

```mermaid
flowchart TD
    A[自动执行代码审查] --> B[生成结构化审查报告]
    B --> C[保存到 docs/code_reviews/ 根目录<br/>状态：待用户处理]
    C --> D[用户检查报告<br/>标记「忽略」或确认修复问题]
    D --> E[用户手动将报告移动到<br/>docs/code_reviews/fixed/ 目录]
    E --> F{下一次自动审查触发}
    F --> G[扫描 fixed/ 目录<br/>提取忽略标记]
    G --> H[去重后追加到 ignored_issues.md<br/>生成 / 更新「已忽略问题清单」]
    H --> I[将 fixed/ 全部报告<br/>归档移动到 archived/ 目录]
    I --> J[清空 fixed/ 目录<br/>仅保留本次新放入的报告]
    J --> K[加载 ignored_issues.md<br/>审查时跳过已忽略问题]
    K --> A
```
